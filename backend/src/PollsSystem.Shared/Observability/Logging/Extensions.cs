...
namespace PollsSystem.Shared.Observability.Logging;
...
public static class Extensions
{
    ...
    {
        services.Configure<SerilogOptions>(configuration.GetSection(DefaultConsts.Logging.SerilogSectionName));
    ...
    private static IHostBuilder AddLogging(this IHostBuilder builder, Action<LoggerConfiguration>? configure = null,
        ...
        => builder.UseSerilog((context, loggerConfiguration) =>
        {
            ...
            {
                loggerSectionName = DefaultConsts.Logging.SerilogSectionName;
            }

            if (string.IsNullOrWhiteSpace(appSectionName))
            {
                appSectionName = DefaultConsts.Logging.AppSectionName;
            }
    ...
    private static void Configure(LoggerConfiguration loggerConfiguration, SerilogOptions options)
    {
        ...
        {
            loggerConfiguration.WriteTo.Console(outputTemplate: DefaultConsts.Logging.ConsoleOutputTemplate);
        }
        ...
        if (fileOptions.Enabled)
        {
            ...
            loggerConfiguration.WriteTo.File(path, rollingInterval: interval, outputTemplate: DefaultConsts.Logging.FileOutputTemplate);
        }
